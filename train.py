...
if __name__ == "__main__":
    ...
    agent = Agent(0.001, 1.0, 2000, 0.00000, 0.99, 512, 30000)
    best_score = 0
    count = 1
    epoch = 0
    epochs = 3000
    while epoch < epochs:
        ...
        while True:
            ...
            if done: 
                if environment.lines > best_score: 
                    ...
                    agent.save_model("models/model" + str(environment.lines) + "-" + str(epoch))
                break  
            print("epochs:", epoch, "count:", count, "lines:", environment.lines, "epsilon:", agent.epsilon, "best score:", best_score, "reward", reward)
            count += 1
            state = next_state
        ...
        epoch += 1
    agent.save_model("models/last_model")
